...
first_frame_path = data_set_root + 'Yoda\\img\\0001.jpg'

ground_truth_boxes = np.loadtxt(data_set_root + 'Yoda\\groundtruth_rect.txt', delimiter = ',')


first_frame = caffe.io.load_image(first_frame_path)

init_box = ground_truth_boxes[:].copy()
...
total_frame_num = 1799 #749
frame_idx = range(955, total_frame_num + 1)
previous_box = init_box

result_boxes = np.zeros((total_frame_num, 5)) # Score, x1, y1, x2, y2
result_boxes[0,0] = 3
...
np.savetxt('test_result_yoda.txt', result_boxes, fmt = '%.2f', delimiter = ',')
